...
- 商品画像: **著作権処理の対象外画像をローカル配信**（AI生成/CC0/PD＋複製。`mock-server/images/<id>-<n>.jpg`。詳細は `mock-server/COPYRIGHT.md`）。第1段階は `python3 -m http.server` で配信していたが、**第2段階では `:server` が `/images` で配信**（アプリは Coil3 で接続先URLから取得）。
...
| モジュール構成 | 単一 `composeApp` モジュール（JetBrains KMP ウィザード方式） | commonMain / androidMain / iosMain |
...
| DI | Koin 4.x | 将来のサーバー実装にも流用 |
| 画像 | Coil 3（coil-compose + coil-network-ktor） | Ktor engine: iOS=Darwin / Android=OkHttp |
| 非同期 | kotlinx-coroutines / StateFlow | |
| シリアライズ | kotlinx-serialization | ナビ引数 + 将来のAPI |
| i18n | 自前 `LocalStrings`（CompositionLocal + `StringsJa`/`StringsEn`） | 実行時トグルを最優先し compose-resources のロケール切替摩擦を回避 |
...
├─ di/AppModule.kt             # Koin モジュール
├─ designsystem/               # theme/color/type/shape + 共通コンポーネント
│  ├─ Theme.kt Color.kt Type.kt Shape.kt Spacing.kt
│  └─ component/               # AppButton, PriceText, ProductCard, RatingStars, QuantityStepper, TopBar, EmptyState ...
├─ i18n/Strings.kt             # LocalStrings + StringsJa/StringsEn + LocaleController
├─ data/
│  ├─ model/                   # Product, Category, Review, CartItem, Order, User, Address, PaymentMethod
│  ├─ mock/MockCatalog.kt      # シード商品・カテゴリ・レビュー（画像URL含む）
│  └─ repository/              # ProductRepository, CartRepository, WishlistRepository,
│                              #   OrderRepository, AuthRepository, AccountRepository（全てインメモリ+StateFlow）
├─ navigation/                 # Routes.kt(@Serializable), AppNavHost.kt, BottomBar.kt
...
androidMain/... : MainActivity, AndroidManifest, Koin初期化, Ktor OkHttp
iosMain/...     : MainViewController.kt, Ktor Darwin
...
- Material 3 ベース。ライト固定を基本（ダークは将来対応の余地を残す）。
...
| 8 | ログイン/サインアップ（モック） | auth | AuthRepository |
...
- **サーバー化での契約変更(注意)**: `CartRepository`/`WishlistRepository`/`AccountRepository` の変更系メソッドは現状**非 `suspend`**（`StateFlow` 即時反映の fire-and-forget）。第2段階でネットワーク往復になるため `suspend` 化が必要（[server-design.md](server-design.md) §9 C1）。
...
- **Phase 1 基盤（逐次・共有契約）**: ① designsystem（theme+主要component）② data(model/mock/repository) ③ i18n(LocalStrings) ④ navigation(Routes/BottomBar/空NavHost)。以降の全機能が依存するため先行確定。
...
- **配信画像は著作権処理の対象外のみ**: AI生成(Pollinations) / CC0・パブリックドメイン / それらの複製。権利処理が必要な画像は削除済み(方針/台帳は `mock-server/COPYRIGHT.md` と `image-provenance.json`)。
...
- **クライアント**: `data/remote`(ApiClient/TokenStore/serverBaseUrl) + `Remote*Repository`（Ktor client で API 呼び出し、StateFlow キャッシュ）。`MockCatalog`/`InMemory*`/`ImageBase` は削除。認証は実認証に置換（「任意情報で成功」は廃止）。
...
- 実決済連携、ダークテーマ、画像のオブジェクトストレージ/CDN 配信、オフライン同梱切替。
</content>
</invoke>
