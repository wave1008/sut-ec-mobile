...
- グラフは**フラット1本**(タブ別の入れ子グラフなし)。カート/商品詳細等はどのタブからでも plain navigate で開く共有画面。
- そのためタブ切替(`AppBottomBar`)で **`saveState`/`restoreState` を使ってはならない**: 保存スタックに共有画面が混ざり、復元でタブ根以外が先頭に出る(ホームタブがカートを開く誤遷移。iOS 通し確認で発現)。タブは常に `popUpTo(startDestination)` + `launchSingleTop` で根へ確定着地させる(タブ内の閲覧状態は保持しない仕様)。
- 「タブごとに続きから再開」を導入するなら、タブ別入れ子グラフへの再編 + 共有画面の所属タブ決定が前提。フラットのまま saveState を足すと再発する。
